...
export class Stickman {
  ...
  _build(room) {
    ...
    const shoulderWidth = headRadius * 2.55;
    const hipWidth = headRadius * 1.95;

    const torsoBottomY = floor - lowerLeg - upperLeg;
    const torsoTopY = torsoBottomY - torsoLength;
    const shoulderY = torsoTopY + torsoLength * 0.16;
    const hipY = torsoBottomY;
    const elbowOffsetX = headRadius * 0.35;
    const handOffsetX = headRadius * 0.48;
    const kneeOffsetX = headRadius * 0.18;
    const footOffsetX = headRadius * 0.3;
    ...
    SIDES.forEach((side) => {
      ...
      const elbowX = shoulderX + sign * (-elbowOffsetX);
      const handX = shoulderX + sign * (-handOffsetX);
      const elbowY = shoulderY + upperArm;
      const handY = elbowY + forearm;
    ...
    SIDES.forEach((side) => {
      ...
      const kneeX = hipX + sign * (-kneeOffsetX);
      const footX = hipX + sign * (-footOffsetX);
